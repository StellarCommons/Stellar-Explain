...
//
// These tests pass synthetic domain objects through the production
// explanation functions (explain_transaction_with_ledger, explain_account_with_org_name)
// and compare the serialized result against checked-in JSON fixtures.
//
// Because the expected output is generated by the real code — not hand-typed —
// the comparison catches wording/logic drift automatically.
//
// Update workflow:
//   1. Run the test — it will fail with a diff if the generator changes.
//   2. Regenerate the fixture: run `cargo test` with the `UPDATE_FIXTURES=1`
//      env var set (or serialize manually via serde_json::to_string_pretty).
...
/// Helper: build a synthetic Transaction with one payment + one skipped op.
fn build_multi_op_transaction() -> stellar_explain_core::models::transaction::Transaction {
    use stellar_explain_core::models::operation::{Operation, OtherOperation, PaymentOperation};
    ...
    let ops = vec![
        Operation::Payment(PaymentOperation {
            id: "op1".to_string(),
            source_account: Some(
                "GCOINBASEAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA".to_string(),
            ),
            destination: "GBINANCEAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA".to_string(),
        ...
        Operation::Other(OtherOperation {
            id: "op2".to_string(),
            operation_type: "manage_data".to_string(),
        }),
    ];
...
/// Helper: build a synthetic Account for explanation testing.
fn build_test_account() -> stellar_explain_core::models::account::Account {
    use stellar_explain_core::models::account::{Account, AccountFlags, Balance};

    Account {
        id: "GAAQCAIBAEAQCAIBAEAQCAIBAEAQCAIBAEAQCAIBAEAQCAIBAEAQDZ7H".to_string(),
        account_id: "GAAQCAIBAEAQCAIBAEAQCAIBAEAQCAIBAEAQCAIBAEAQCAIBAEAQDZ7H".to_string(),
        ...
        balances: vec![
            ...
            Balance {
                ...
                asset_issuer: Some(
                    "GA5ZSEJYB37JDD5G4LYQCI7ARTUOEYRCIEJXE4G6NYRTHG7V2CUVBK2T".to_string(),
                ),
...
fn snapshot_transaction_explanation_payment() {
    use stellar_explain_core::explain::transaction::explain_transaction_with_ledger;
    use stellar_explain_core::models::fee::FeeStats;

    let tx = build_multi_op_transaction();
    let fee_stats = FeeStats::default_network_fees();

    let actual = explain_transaction_with_ledger(
        &tx,
        Some(&fee_stats),
        Some("2024-01-15T14:32:00Z"),
        Some(49823145),
    )
    .expect("explain_transaction_with_ledger failed");
...
fn snapshot_account_explanation() {
    use stellar_explain_core::explain::account::explain_account_with_org_name;
    use stellar_explain_core::routes::account::AccountExplanationResponse;

    let account = build_test_account();
    let explanation = explain_account_with_org_name(&account, None);

    let response = AccountExplanationResponse {
...
fn snapshot_account_history() {
    ...
    // AccountHistoryResponse fields are mapped directly from Horizon records
    // in the route handler (no standalone generator function). This snapshot
    // verifies that the struct shape + serialization matches the fixture.
    let response = AccountHistoryResponse {
